...
abstract contract SimpleQuinaryTree {
     ...
     */
    function getZero(uint256 _level) internal virtual view returns (uint256) {}
    ...
    function enqueue(uint256 _leaf) internal {
    ...
    function update(uint256 _index, uint256 _leaf) internal {
    ...
    function _updateAt(uint256 _index) private {
        ...
        while (idx > 0) {
            ...
            for (uint256 i = 0; i < 5; i++) {
                uint256 child = nodes[childrenIdx0];
                if (child == 0) {
                    child = zero;
                }
                inputs[i] = child;
